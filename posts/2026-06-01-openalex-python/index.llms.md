...
論文を検索するには、`/works`エンドポイントに対してGETリクエストを送ります。 ほかのエンドポイントは、以下のOpenAlex APIのドキュメントを参照してください。
...
デフォルトでは、関連順（relevance）でソートされているため、キーワードとの関連性が高い順に結果が表示されます。 もし、被引用数の多い順に並べ替えたい場合は、クエリパラメータに `sort=cited_by_count:desc`を追加するなどして、並べ替えの順番を指定することもできます。
