...
description: "OpenAlex APIをPythonから呼び出し、論文メタデータを取得するための下書きです。"
date: "2026-06-01"
...
::: {.callout-note}
OpenAlexのAPI Referenceでは、`api_key` をクエリパラメータとして渡す形式が案内されています。
GitHub Actionsで実行する場合は、`OPENALEX_API_KEY` をRepository secretとして登録し、workflow側で環境変数として渡します。
:::
...
次のコードは、`/works` エンドポイントに対してキーワード検索を行う例です。
APIキーが必要なので、下書き段階では `eval: false` にしています。
実際に記事として実行する場合は、APIキーを設定したうえで `eval: true` に変更します。
