---
title: "Pythonで最低限のuvによる環境管理"
description: "Pythonの環境管理ツールであるuvを使用して、最低限の環境管理方法をまとめます"
date: "2026-03-20"
...
::: {.callout-note}
English version: [Minimal Python Environment Management with uv](../../en/posts/2026-03-20-uv-python/index.qmd)
:::

## uvとは

[uv](https://docs.astral.sh/uv/)とは、Pythonの環境管理ツールです。
他の環境管理ツールには、[venv](https://docs.python.org/3/library/venv.html)や[virtualenv](https://virtualenv.pypa.io/en/latest/)などがあります。

uvは他の環境管理と比べても新し目で、速度や安定性、使いやすさなどが特徴です。
筆者もこれまではvenvを使用していましたが、uvに乗り換えてからは速度や安定性の向上を感じています。
...
大規模なプロジェクトでももちろん使用する恩恵が大きいですが、筆者のような小規模な個人開発でも、環境管理の手間が減るため、非常におすすめです。
今回は、筆者のよく使う、小規模な個人開発向けの最低限で汎用性のあるuvによる環境管理方法をまとめます。
...
このコマンドで、`numpy`というパッケージがプロジェクトに追加されます。
追加されたパッケージは、`pyproject.toml`ファイルに記載されます。
